...
exports.accounting = async (req) => {
  ...
  try {
    ...
    for (const productListitem of cartItems) {
      if (productListitem.product.status !== "D") {
        if (productListitem.product.status === "P") {
          productListitem.product = productListitem.product.oldDetails;
          productListitem.product.oldDetails = null;
        }
    ...
    for (let vendor of Object.keys(itemsGroupedByVendor)) {
      ...
      for (const productListitem of cartItems) {
        ...
        console.log("Product Details:", productListitem.product);
        ...
        if (productListitem.product.vendor.toString() === vendor) {
          ...
          console.log(
            `Weight: ${weight}, Shipping Cost for First Product: ${unitShippingCost_firstProduct}`
          );
          ...
          console.log(
            `Vendor Shipping Cost (before adding to vendorShippingCosts): ${vendorShipping}`
          );
    ...
    console.log(
      `Vendor-wise Shipping Costs: ${JSON.stringify(vendorShippingCosts)}`
    );
    ...
    console.log(`Final Order Accounting: ${JSON.stringify(orderAccounting)}`);
    
    // Cart Clear
    await makeMongoDbServiceCart.bulkUpdate(
			{ user: req.user._id },
			{ $set: { cartItems: [] } }
		);
    return response(
